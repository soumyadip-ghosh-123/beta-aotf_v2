...
`onboardingCompleted` is set to `true` by `/api/v1/payments/verify` after Razorpay payment. For legacy migrated users, it is set during the `user.created` webhook by detecting `migratedFromLegacy: true`.
